...
1. **Is the center of gravity many concurrent, long-lived websockets?** Yes: dedicated WS server (Phoenix or Go), Next.js only as the web client. No: continue.
2. **Is it a native mobile app with real device capabilities?** Yes: Expo or native, Next.js only as site and API. No: continue.
3. **Does the core work run for minutes, or need GPU / heavy ML?** Yes: Python service plus queue and workers, Next.js as the thin front. No: continue.
4. **Is it purely static prose with no authenticated surface and no per-request logic?** Yes: Astro. No: continue.
5. **Is it one or a few endpoints with no UI?** Yes: a single serverless function or Hono. No: continue.
6. **Otherwise:** it is a web app that renders HTML and needs server logic. Use Next.js. This is the default, and reaching this line means it is the right one.
